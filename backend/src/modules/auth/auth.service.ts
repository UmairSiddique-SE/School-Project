...
export class AuthService {
    ...
    try {
      ...
      const result = await this.prisma.$transaction(async (tx) => {
        const school = await tx.school.create({
          data: {
            name: dto.schoolName.trim(), slug: schoolSlug, type: dto.schoolType,
            logoUrl: dto.logoUrl, phone: dto.schoolPhone || dto.adminPhone,
            address: dto.schoolAddress, country: dto.country, city: dto.city, isActive: false,
          },
        });
        const user = await tx.user.create({
          data: {
            name: dto.adminName.trim(), email: adminEmail, passwordHash,
            role: 'SCHOOL_ADMIN', schoolId: school.id, phone: dto.adminPhone,
          },
        });
        await tx.subscription.create({
          data: {
            schoolId: school.id, plan: dto.requestedPlan || 'FREE_TRIAL', status: 'PENDING',
            endDate: new Date(Date.now() + 14 * 24 * 60 * 60 * 1000), amount: 0,
          },
        });
      ...
      this.mailService.sendEmailVerification(result.user.email, otp).catch((error) => {
        console.error('Failed to send verification email:', error);
      });

      return {
        message: 'School registered. Verify your email to continue.', verificationRequired: true,
        verificationUserId: result.user.id,
        user: { id: result.user.id, name: result.user.name, email: result.user.email,
          role: result.user.role, schoolId: result.user.schoolId,
          schoolName: result.school.name, schoolSlug: result.school.slug },
      };
    } catch (error: any) {
  ...
  async login(dto: LoginDto) {
    ...
    if (user.school?.subscription && (user.school.subscription.status === 'EXPIRED' || user.school.subscription.endDate < new Date())) {
      throw new UnauthorizedException('Your subscription has expired. Please renew your plan.');
    }

    const tokens = await this.generateTokens(user.id, user.email, user.role, user.schoolId ?? undefined);
    return { user: { id: user.id, name: user.name, email: user.email, role: user.role,
      schoolId: user.schoolId, schoolName: user.school?.name, schoolSlug: user.school?.slug,
      activationStatus: user.school?.isActive ? 'ACTIVE' : 'PAYMENT_PENDING', plan: user.school?.subscription?.plan }, ...tokens };
  }
  ...
  async refreshToken(token: string) {
    ...
    if (!stored.user.isActive || (stored.user.school && (!stored.user.school.isActive || stored.user.school.subscription?.status === 'EXPIRED' || (stored.user.school.subscription?.endDate && stored.user.school.subscription.endDate < new Date())))) {
  ...
  async verifyEmail(dto: VerifyEmailDto) {
    ...
    return { message: 'Email verified successfully', user: { id: user.id, name: user.name, email: user.email,
      role: user.role, schoolId: user.schoolId, schoolName: user.school?.name, schoolSlug: user.school?.slug,
      activationStatus: user.school?.isActive ? 'ACTIVE' : 'PAYMENT_PENDING', plan: user.school?.subscription?.plan }, ...tokens };
  }
  ...
  async updateProfile(userId: string, dto: UpdateProfileDto) {
    return this.prisma.user.update({ where: { id: userId }, data: { name: dto.name.trim(), phone: dto.phone?.trim() || null }, select: { id: true, name: true, email: true, role: true, phone: true, schoolId: true } });
  }
