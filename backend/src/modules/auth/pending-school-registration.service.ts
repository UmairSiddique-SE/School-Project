...
export class PendingSchoolRegistrationService {
  ...
  private endDate(start: Date, period: string) {
    ...
    if (normalized.includes('forever') || normalized.includes('unlimited')) {
      return new Date(FOREVER_DATE);
    }
  ...
  private async findPending(id: string) {
    const rows = await this.prisma.$queryRaw<PendingRow[]>`
      SELECT "id", "schoolName", "schoolSlug", "schoolType", "logoUrl",
             "schoolAddress", "schoolPhone", "country", "city", "adminName",
             "adminEmail", "adminPhone", "passwordHash", "requestedPlan",
  ...
  async register(dto: RegisterSchoolDto) {
    ...
    await this.prisma.$executeRaw`
      INSERT INTO "PendingSchoolRegistration"
        ("id", "schoolName", "schoolSlug", "schoolType", "logoUrl",
         "schoolAddress", "schoolPhone", "country", "city", "adminName",
         "adminEmail", "adminPhone", "passwordHash", "requestedPlan",
         "otp", "otpExpiresAt")
      VALUES
        (${id}, ${dto.schoolName.trim()}, ${schoolSlug}, ${dto.schoolType},
         ${dto.logoUrl}, ${dto.schoolAddress}, ${dto.schoolPhone}, ${dto.country},
         ${dto.city}, ${dto.adminName.trim()}, ${adminEmail}, ${dto.adminPhone},
         ${passwordHash}, ${planKey}, ${otp}, ${otpExpiresAt})
      ON CONFLICT ("schoolSlug") DO UPDATE SET
        ...
        "adminPhone" = EXCLUDED."adminPhone",
        "passwordHash" = EXCLUDED."passwordHash",
        "requestedPlan" = EXCLUDED."requestedPlan",
        "otp" = EXCLUDED."otp",
    ...
    if (!registration) {
      throw new BadRequestException('Unable to create registration session');
    }

    const emailSent = await this.mailService.sendEmailVerification(adminEmail, otp);
    if (!emailSent) {
      throw new BadRequestException(
        'We could not send the verification email. Please check the mail configuration and try again.',
      );
    }
  ...
  async resend(id: string) {
    ...
    if (pending.verifiedAt) {
      throw new BadRequestException(
        'Email is already verified. Please continue with your registration.',
      );
    }
  ...
  async verify(id: string, otp: string) {
    ...
    if (pending.verifiedAt) {
      throw new BadRequestException(
        'Email already verified. Please continue with your registration.',
      );
    }

    const submittedOtp = this.normalizeOtp(otp);
    const storedOtp = this.normalizeOtp(pending.otp);
    ...
    if (storedOtp !== submittedOtp) {
      throw new BadRequestException(
        'Invalid OTP. Please enter the latest 6-digit code sent to your email.',
      );
    }

    if (pending.otpExpiresAt.getTime() <= Date.now()) {
      throw new BadRequestException(
        'OTP has expired. Please resend a new verification code.',
      );
    }

    const plan = await this.prisma.platformPlan.findUnique({
      where: { planKey: pending.requestedPlan },
    });
    if (!plan || !plan.isActive) {
      throw new BadRequestException('Selected subscription plan is unavailable');
    }
    ...
    const result = await this.prisma.$transaction(async (tx) => {
      const existingUser = await tx.user.findUnique({
        where: { email: pending.adminEmail },
        select: { id: true },
      });
      if (existingUser) {
        throw new ConflictException('Email already registered');
    ...
    await this.prisma.$executeRaw`
      DELETE FROM "PendingSchoolRegistration"
      WHERE "id" = ${id}
    `;
