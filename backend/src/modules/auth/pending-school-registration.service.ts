...
export class PendingSchoolRegistrationService {
  ...
  private async findPending(id: string) {
    const rows = await this.prisma.$queryRaw<PendingRow[]>`
      SELECT "id", "schoolName", "schoolSlug", "schoolType", "logoUrl", "schoolAddress", "schoolPhone",
             "country", "city", "adminName", "adminEmail", "adminPhone", "passwordHash", "requestedPlan",
             "otp", "otpExpiresAt", "verifiedAt"
      FROM "PendingSchoolRegistration"
      WHERE "id" = ${id}
      LIMIT 1
    `;
    return rows[0] || null;
  ...
  async register(dto: RegisterSchoolDto) {
    ...
    await this.prisma.$executeRaw`
      INSERT INTO "PendingSchoolRegistration"
      ("id", "schoolName", "schoolSlug", "schoolType", "logoUrl", "schoolAddress", "schoolPhone", "country", "city",
       "adminName", "adminEmail", "adminPhone", "passwordHash", "requestedPlan", "otp", "otpExpiresAt")
      VALUES (${id}, ${dto.schoolName.trim()}, ${schoolSlug}, ${dto.schoolType}, ${dto.logoUrl}, ${dto.schoolAddress}, ${dto.schoolPhone},
              ${dto.country}, ${dto.city}, ${dto.adminName.trim()}, ${adminEmail}, ${dto.adminPhone}, ${passwordHash}, ${planKey},
              ${otp}, ${new Date(Date.now() + 15 * 60 * 1000)})
      ON CONFLICT ("schoolSlug") DO UPDATE SET
        "schoolName" = EXCLUDED."schoolName", "schoolType" = EXCLUDED."schoolType", "logoUrl" = EXCLUDED."logoUrl",
        "schoolAddress" = EXCLUDED."schoolAddress", "schoolPhone" = EXCLUDED."schoolPhone", "country" = EXCLUDED."country",
        "city" = EXCLUDED."city", "adminName" = EXCLUDED."adminName", "adminEmail" = EXCLUDED."adminEmail",
        "adminPhone" = EXCLUDED."adminPhone", "passwordHash" = EXCLUDED."passwordHash", "requestedPlan" = EXCLUDED."requestedPlan",
        "otp" = EXCLUDED."otp", "otpExpiresAt" = EXCLUDED."otpExpiresAt", "verifiedAt" = NULL, "updatedAt" = CURRENT_TIMESTAMP
    `;

    const pending = await this.prisma.$queryRaw<PendingRow[]>`
      SELECT "id", "schoolName", "schoolSlug", "schoolType", "logoUrl", "schoolAddress", "schoolPhone",
             "country", "city", "adminName", "adminEmail", "adminPhone", "passwordHash", "requestedPlan",
             "otp", "otpExpiresAt", "verifiedAt"
      FROM "PendingSchoolRegistration" WHERE "schoolSlug" = ${schoolSlug} LIMIT 1
    `;
    const registration = pending[0];
    if (!registration) throw new BadRequestException('Unable to create registration session');

    this.mailService.sendEmailVerification(adminEmail, otp).catch((error) => console.error('Failed to send verification email:', error));
    return {
      message: 'School registration started. Verify your email to continue.',
      verificationRequired: true,
      verificationUserId: registration.id,
      schoolSlug: registration.schoolSlug,
  ...
  async resend(id: string) {
    ...
    const otp = String(randomInt(100000, 1000000));
    await this.prisma.$executeRaw`
      UPDATE "PendingSchoolRegistration"
      SET "otp" = ${otp}, "otpExpiresAt" = ${new Date(Date.now() + 15 * 60 * 1000)}, "updatedAt" = CURRENT_TIMESTAMP
      WHERE "id" = ${id}
    `;
    this.mailService.sendEmailVerification(pending.adminEmail, otp).catch((error) => console.error('Failed to resend verification email:', error));
  ...
  async verify(id: string, otp: string) {
    ...
    if (pending.otp !== otp.trim() || pending.otpExpiresAt < new Date())
      throw new BadRequestException('Invalid or expired OTP. Please use the latest code or resend OTP.');
    ...
    const result = await this.prisma.$transaction(async (tx) => {
      const school = await tx.school.create({
        data: {
          ...
          slug: pending.schoolSlug,
          type: pending.schoolType,
          logoUrl: pending.logoUrl,
          phone: pending.schoolPhone,
          address: pending.schoolAddress,
      ...
      await tx.subscription.create({
        data: {
          ...
          plan: plan.planKey,
          status: 'PENDING',
          endDate: this.endDate(new Date(), plan.period),
          amount: plan.price,
          currency: plan.currency,
        },
      });
      await tx.schoolRequest.create({
        data: {
          schoolName: school.name,
          ownerName: user.name,
          email: user.email,
          phone: pending.adminPhone,
          whatsapp: pending.adminPhone,
          city: pending.city,
          address: pending.schoolAddress,
          subdomain: pending.schoolSlug,
