  ...
  async review(
    ...
    if (action === 'REJECTED') {
      return this.prisma.$transaction(async (tx) => {
        ...
        if (reviewerUserId) {
          await tx.auditLog.create({ data: { action: 'SCHOOL_REQUEST_REJECTED', entity: 'SchoolRequest', entityId: id, userId: reviewerUserId, schoolId: existingSchool?.id, after: `Rejected registration request for ${request.schoolName}` } });
        }
        return updated;
      });
    }

    if (!existingSchool) throw new ConflictException('This request has no matching registered school account. Please ask the school to register again.');
    ...
    const result = await this.prisma.$transaction(async (tx) => {
      ...
      if (subscription) {
        ...
        await tx.subscription.create({ data: { schoolId: existingSchool.id, plan: plan.planKey, status: 'ACTIVE', startDate, endDate, amount: plan.price, currency: plan.currency } });
      }
      ...
      return { updatedRequest, subscription: await tx.subscription.findUnique({ where: { schoolId: existingSchool.id } }), canActivate: true };
