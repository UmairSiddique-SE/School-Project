...
export class SchoolApprovalService {
  ...
  async review(id: string, action: 'APPROVED' | 'REJECTED', reviewNotes?: string, reviewedBy?: string, reviewerUserId?: string) {
    ...
    const result = await this.prisma.$transaction(async (tx) => {
      ...
      // Paid plans stay PAYMENT_PENDING until the separate payment review approves them.
      ...
      const subscription = await tx.subscription.create({ data: { schoolId: school.id, plan: plan.planKey, status: isFreeTrial ? 'ACTIVE' : 'PENDING', endDate: subscriptionEnd, amount: plan.price, currency: plan.currency } });
      const updatedRequest = await tx.schoolRequest.update({ where: { id }, data: { status: 'APPROVED', reviewNotes: reviewNotes || null, reviewedBy: reviewedBy || 'Super Admin', reviewedAt: now } });
      let finalReviewerId = reviewerUserId;
      if (!finalReviewerId) finalReviewerId = (await tx.user.findFirst({ where: { role: 'SUPER_ADMIN' } }))?.id;
      if (finalReviewerId) await tx.auditLog.create({ data: { action: 'CREATE', entity: 'School', entityId: school.id, after: `Approved registration request for ${request.schoolName}. Created school ID: ${school.id}. ${isFreeTrial ? 'Free trial active.' : 'Paid plan awaiting payment approval.'}`, userId: finalReviewerId } });
