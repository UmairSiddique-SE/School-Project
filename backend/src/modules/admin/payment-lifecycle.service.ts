...
export class PaymentLifecycleService implements OnModuleInit, OnModuleDestroy {
  ...
  async approvePayment(id: string, actor?: any) {
    return this.prisma.$transaction(async (tx) => {
      ...
      if (schoolAdmin?.email) await tx.schoolRequest.updateMany({ where: { email: schoolAdmin.email, status: 'PENDING' }, data: { status: 'APPROVED', reviewedBy: actor?.name || 'Super Admin', reviewedAt: now, reviewNotes: 'Payment approved by Super Admin.' } });
