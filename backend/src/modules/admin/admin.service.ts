...
const DEFAULT_PLANS = [
  { planKey: 'FREE_TRIAL', name: 'Free Trial', price: 0, currency: 'PKR', period: '1 day', maxStudents: 20, maxTeachers: 15, storageMb: 1024, supportTier: 'Email', features: JSON.stringify(['Up to 20 students', 'Up to 15 staff', '1 campus', 'Core features', 'Email support']) },
...
const DEFAULT_SETTINGS = [
  ...
  { key: 'subscription.trialDays', value: '1', type: 'number', label: 'Free Trial Duration (days)', description: 'How many days the free trial lasts before expiry.', category: 'subscription' },
...
export class AdminService {
  ...
  async reviewSchoolRequest(id: string, action: 'APPROVED' | 'REJECTED', reviewNotes?: string, reviewedBy?: string, reviewerUserId?: string) {
    const request = await this.prisma.schoolRequest.findUnique({ where: { id } }); if (!request) throw new NotFoundException('School request not found'); if (request.status !== 'PENDING') throw new ConflictException('This request has already been reviewed');
    if (action === 'REJECTED') return this.prisma.$transaction(async (tx) => { const updated = await tx.schoolRequest.update({ where: { id }, data: { status: 'REJECTED', reviewNotes: reviewNotes || null, reviewedBy: reviewedBy || 'Super Admin', reviewedAt: new Date() } }); if (reviewerUserId) await tx.auditLog.create({ data: { action: 'SCHOOL_REQUEST_REJECTED', entity: 'SchoolRequest', entityId: id, userId: reviewerUserId, after: `Rejected registration request for ${request.schoolName}` } }); return updated; });
    const existingUser = await this.prisma.user.findUnique({ where: { email: request.email } }); if (existingUser) throw new ConflictException('The contact email is already registered to an existing user');
    let baseSlug = request.schoolName.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/(^-|-$)/g, ''); if (!baseSlug) baseSlug = 'school'; let slug = baseSlug; let index = 1; while (await this.prisma.school.findUnique({ where: { slug } })) slug = `${baseSlug}-${index++}`;
    const tempPassword = Math.random().toString(36).slice(-10); const passwordHash = await bcrypt.hash(tempPassword, 12);
    const result = await this.prisma.$transaction(async (tx) => {
      const school = await tx.school.create({ data: { name: request.schoolName, slug, email: request.email, phone: request.phone, city: request.city, address: request.address, country: 'Pakistan', isActive: true } });
      const user = await tx.user.create({ data: { name: request.ownerName, email: request.email, passwordHash, role: 'SCHOOL_ADMIN', schoolId: school.id, isActive: true, mustChangePassword: true, emailVerified: true } });
      const plan = request.requestedPlan || 'FREE_TRIAL'; const platformPlan = await tx.platformPlan.findUnique({ where: { planKey: plan } }); const days = plan === 'FREE_TRIAL' ? 1 : 30;
      const subscription = await tx.subscription.create({ data: { schoolId: school.id, plan, status: 'ACTIVE', endDate: new Date(Date.now() + days * 86400000), amount: platformPlan?.price || 0, currency: platformPlan?.currency || 'PKR' } });
      ...
      let finalReviewerId = reviewerUserId; if (!finalReviewerId) finalReviewerId = (await tx.user.findFirst({ where: { role: 'SUPER_ADMIN' } }))?.id; if (finalReviewerId) await tx.auditLog.create({ data: { action: 'CREATE', entity: 'School', entityId: school.id, after: `Approved registration request for ${request.schoolName}. Created school ID: ${school.id}`, userId: finalReviewerId } });
      return { school, user, subscription, updatedRequest };
    });
    this.mailService.sendSchoolOnboarding(result.user.email, { schoolName: result.school.name, schoolSlug: result.school.slug, adminName: result.user.name, temporaryPassword: tempPassword, plan: result.subscription.plan }).catch((err) => console.error('Failed to send onboarding email:', err)); return result.updatedRequest;
  }
  ...
  async getAuditLogs(action?: string, search?: string, page?: number, limit?: number) {
    ...
    if (page !== undefined && limit !== undefined) { const skip = (page - 1) * limit; const [total, data] = await Promise.all([this.prisma.auditLog.count({ where }), this.prisma.auditLog.findMany({ where, orderBy: { createdAt: 'desc' }, skip, take: limit, include: { user: { select: { name: true, email: true, role: true } }, school: { select: { name: true } } } })]); return { data: data.map((log) => ({ id: log.id, action: log.action, entity: log.entity, entityId: log.entityId, entityName: log.school?.name || log.entity, actor: log.user?.name || 'System', actorEmail: log.user?.email || '', details: log.after || log.action, ip: log.ipAddress || 'N/A', timestamp: log.createdAt })), meta: { total, page, limit, totalPages: Math.ceil(total / limit) } } as any; }
    const logs = await this.prisma.auditLog.findMany({ where, orderBy: { createdAt: 'desc' }, take: 200, include: { user: { select: { name: true, email: true, role: true } }, school: { select: { name: true } } } }); return logs.map((log) => ({ id: log.id, action: log.action, entity: log.entity, entityId: log.entityId, entityName: log.school?.name || log.entity, actor: log.user?.name || 'System', actorEmail: log.user?.email || '', details: log.after || log.action, ip: log.ipAddress || 'N/A', timestamp: log.createdAt }));
  }
  ...
  async approvePayment(id: string, actor?: any) { const payment = await this.prisma.onboardingPayment.findUnique({ where: { id } }); if (!payment) throw new NotFoundException('Payment not found'); if (payment.status !== 'PENDING') throw new BadRequestException('Only pending payments can be approved'); return this.prisma.$transaction(async (tx) => { const approved = await tx.onboardingPayment.update({ where: { id }, data: { status: 'APPROVED', reviewedAt: new Date() } }); await tx.school.update({ where: { id: payment.schoolId }, data: { isActive: true } }); const plan = await tx.platformPlan.findUnique({ where: { planKey: payment.plan } }); await tx.subscription.update({ where: { schoolId: payment.schoolId }, data: { plan: payment.plan, status: 'ACTIVE', startDate: new Date(), endDate: addDays(new Date(), 365), amount: payment.amount, currency: plan?.currency || 'PKR' } }); if (actor?.id) await tx.auditLog.create({ data: { action: 'PAYMENT_APPROVED', entity: 'OnboardingPayment', entityId: id, after: `Approved PKR ${payment.amount} for ${payment.plan}`, schoolId: payment.schoolId, userId: actor.id } }); await tx.notification.createMany({ data: (await tx.user.findMany({ where: { schoolId: payment.schoolId, isActive: true }, select: { id: true } })).map((u) => ({ type: 'PAYMENT', title: 'Payment approved', message: `Your ${payment.plan} subscription payment has been approved.`, schoolId: payment.schoolId, userId: u.id })) }); return approved; }); }
  async rejectPayment(id: string, actor?: any) { const payment = await this.prisma.onboardingPayment.findUnique({ where: { id } }); if (!payment) throw new NotFoundException('Payment not found'); if (payment.status !== 'PENDING') throw new BadRequestException('Only pending payments can be rejected'); return this.prisma.$transaction(async (tx) => { const rejected = await tx.onboardingPayment.update({ where: { id }, data: { status: 'REJECTED', reviewedAt: new Date() } }); if (actor?.id) await tx.auditLog.create({ data: { action: 'PAYMENT_REJECTED', entity: 'OnboardingPayment', entityId: id, after: `Rejected PKR ${payment.amount} for ${payment.plan}`, schoolId: payment.schoolId, userId: actor.id } }); return rejected; }); }
  ...
  async getReportCsv(id: string): Promise<string> {
    ...
    if (id === 'expiry-report') { const now = new Date(); const thirtyDays = new Date(Date.now() + 30 * 86400000); const schools = await this.prisma.school.findMany({ where: { deletedAt: null, subscription: { endDate: { gte: now, lte: thirtyDays } } }, include: { subscription: true } }); let csv = 'SchoolID,SchoolName,Plan,ExpiryDate,Email\n'; for (const s of schools) csv += `"${s.id}","${s.name}","${s.subscription?.plan}","${s.subscription?.endDate.toISOString()}","${s.email || ''}"\n`; return csv; }
