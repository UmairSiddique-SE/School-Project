...
export class AdminService {
  ...
  async seedDefaults() {
    ...
    if (settingCount === 0) {
      for (const s of DEFAULT_SETTINGS) await this.prisma.platformSetting.upsert({ where: { key: s.key }, create: s, update: {} });
    }
    const templateCount = await this.prisma.emailTemplate.count();
    if (templateCount === 0) {
      for (const t of DEFAULT_TEMPLATES) await this.prisma.emailTemplate.create({ data: t });
    }
  }
  ...
  async reviewSchoolRequest(id: string, action: 'APPROVED' | 'REJECTED', reviewNotes?: string, reviewedBy?: string, reviewerUserId?: string) {
    ...
    let slug = baseSlug; let index = 1;
    while (true) { const existingSchool = await this.prisma.school.findUnique({ where: { slug } }); if (!existingSchool) break; slug = `${baseSlug}-${index++}`; }
    ...
    const result = await this.prisma.$transaction(async (tx) => {
      ...
      const days = plan === 'FREE_TRIAL' ? 1 : 30;
      const subscription = await tx.subscription.create({ data: { schoolId: school.id, plan, status: 'ACTIVE', endDate: new Date(Date.now() + days * 24 * 60 * 60 * 1000), amount: platformPlan?.price || 0, currency: platformPlan?.currency || 'PKR' } });
      ...
      if (!finalReviewerId) { const defaultAdmin = await tx.user.findFirst({ where: { role: 'SUPER_ADMIN' } }); finalReviewerId = defaultAdmin?.id; }
      if (finalReviewerId) await tx.auditLog.create({ data: { action: 'CREATE', entity: 'School', entityId: school.id, after: `Approved registration request for ${request.schoolName}. Created school ID: ${school.id}`, userId: finalReviewerId } });
      return { school, user, subscription, updatedRequest };
    });
    this.mailService.sendSchoolOnboarding(result.user.email, { schoolName: result.school.name, schoolSlug: result.school.slug, adminName: result.user.name, temporaryPassword: tempPassword, plan: result.subscription.plan }).catch((err) => console.error('Failed to send onboarding email:', err));
  ...
  async getAuditLogs(action?: string, search?: string, page?: number, limit?: number) {
    ...
    if (search) where.OR = [{ entity: { contains: search } }, { action: { contains: search } }, { user: { name: { contains: search } } }];
