...
export class PeopleController {
  constructor(private readonly peopleService: PeopleService, private readonly planLimitService: PlanLimitService, private readonly prisma: PrismaService) {}

  @Get('me')
  async getMe(@CurrentUser() user: any) {
    if (user?.role === 'PARENT') {
      const account = await this.prisma.user.findFirst({ where: { id: user.id, schoolId: user.schoolId, role: 'PARENT', isActive: true, deletedAt: null }, select: { id: true, name: true, email: true, role: true, phone: true, avatarUrl: true, schoolId: true } });
      if (!account) return null;
      const parent = await this.prisma.parent.findFirst({ where: { userId: account.id, schoolId: user.schoolId, deletedAt: null }, include: { students: { include: { student: { include: { section: { include: { class: true } }, attendances: { orderBy: { date: 'desc' }, take: 100 }, examResults: { include: { exam: true, subject: true }, orderBy: { id: 'desc' }, take: 100 }, feePayments: { take: 50 }, homeworkSubmissions: { include: { homework: true }, orderBy: { id: 'desc' }, take: 50 } } } } } });
  ...
  @Post('students') @Roles('SCHOOL_ADMIN')
  async createStudent(@CurrentUser() user: any, @Body() dto: any) {
    ...
    const { password: _password, ...studentDto } = dto; void _password;
    return this.peopleService.createStudent(user.schoolId, studentDto);
  }

  @Patch('students/:id') @Roles('SCHOOL_ADMIN')
  ...
  @Delete('students/:id') @Roles('SCHOOL_ADMIN')
  async deleteStudent(@CurrentUser() user: any, @Param('id') id: string) {
    if (!id?.trim()) throw new BadRequestException('Student ID is required');
    const student = await this.prisma.student.findFirst({ where: { id, schoolId: user.schoolId, deletedAt: null }, select: { id: true } });
  ...
  @Post('students/promote') @Roles('SCHOOL_ADMIN')
  async promoteStudents(@CurrentUser() user: any, @Body() dto: any) {
    ...
    const target = await this.prisma.section.findFirst({ where: { id: dto.sectionId, deletedAt: null, class: { schoolId: user.schoolId, deletedAt: null } }, select: { id: true, capacity: true, classId: true } });
  ...
  @Post('students/transfer') @Roles('SCHOOL_ADMIN')
  ...
  private validateStudentPayload(dto: any, creating: boolean) {
    ...
    if (creating && (!dto.name || typeof dto.name !== 'string' || dto.name.trim().length < 2 || dto.name.trim().length > 150)) throw new BadRequestException('Student name is required and must contain 2-150 characters');
    if (!creating && dto.name !== undefined && (typeof dto.name !== 'string' || dto.name.trim().length < 2 || dto.name.trim().length > 150)) throw new BadRequestException('Student name must contain 2-150 characters');
    ...
    if (dto.email !== undefined && dto.email !== null && dto.email !== '') { const email = String(dto.email).trim().toLowerCase(); if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) throw new BadRequestException('Please provide a valid student email address'); dto.email = email; }
    if (dto.dateOfBirth !== undefined && dto.dateOfBirth !== null && dto.dateOfBirth !== '') { const date = new Date(dto.dateOfBirth); if (Number.isNaN(date.getTime())) throw new BadRequestException('Invalid date of birth'); if (date > new Date()) throw new BadRequestException('Date of birth cannot be in the future'); }
    if (dto.bFormNumber !== undefined && dto.bFormNumber !== null && dto.bFormNumber !== '') { const bForm = String(dto.bFormNumber).trim(); if (!/^\d{5}-\d{7}-\d$/.test(bForm)) throw new BadRequestException('B-Form / CNIC must use format 35202-1234567-1'); dto.bFormNumber = bForm; }
    if (dto.rollNo !== undefined && dto.rollNo !== null && dto.rollNo !== '') { const roll = String(dto.rollNo).trim(); if (!/^\d+$/.test(roll) || Number(roll) < 1) throw new BadRequestException('Roll number must be a positive number'); dto.rollNo = roll; }
    ...
    for (const field of ['phone', 'studentMobile', 'emergencyContact']) if (dto[field] !== undefined && dto[field] !== null && dto[field] !== '' && String(dto[field]).length > 30) throw new BadRequestException(`${field} is too long`);
    if (creating && (dto.fatherName || dto.motherName || dto.guardianName) && (!dto.parentPassword || String(dto.parentPassword).length < 12)) throw new BadRequestException('Parent password must contain at least 12 characters');
  }
