...
PeopleService.prototype.deleteStaff = async function (id: string, schoolId: string) {
  ...
  if (staff) {
    return service.prisma.$transaction(async (tx: any) => {
      ...
      await tx.user.updateMany({ where: { email: staff.email, schoolId }, data: { isActive: false, deletedAt: new Date() } });
    });
  }
...
PeopleService.prototype.getSchoolStats = async function (schoolId: string) {
  ...
  const pendingFees = feePayments.filter((p: any) => p.status === 'PENDING' || p.status === 'OVERDUE').reduce((acc: number, curr: any) => acc + (curr.amount - curr.totalPaid), 0);
...
PeopleService.prototype.createStudent = async function (schoolId: string, data: any) {
  ...
  const payload = hasParentDetails && !data.parentPassword ? { ...data, parentPassword: randomBytes(24).toString('base64url') } : data;
  const result = await originalCreateStudent.call(this, schoolId, payload);
  const service = this as any;

  if (result?.student?.id && data.avatarUrl) {
    await service.prisma.student.update({ where: { id: result.student.id }, data: { avatarUrl: String(data.avatarUrl) } });
  }

  if (!hasParentDetails || !result?.student?.id) return result;
  const link = await service.prisma.studentParent.findFirst({ where: { studentId: result.student.id, isPrimary: true }, select: { parentId: true } });
  if (!link) return result;
  const parent = await service.prisma.parent.findFirst({ where: { id: link.parentId, schoolId }, select: { id: true, userId: true } });
...
PeopleService.prototype.updateStudent = async function (id: string, schoolId: string, data: any) {
  ...
  const normalized = {
    ...
    currentAddress: data.currentAddress ?? data.address ?? undefined,
  ...
  const result = await originalUpdateStudent.call(this, id, schoolId, normalized);
  const extra: any = {};
  const studentKeys = ['avatarUrl', 'leavingCertificateUrl', 'previousAcademicRecord', 'medicalNotes', 'specialRequirements'];
  for (const key of studentKeys) if (data[key] !== undefined) extra[key] = data[key];
  if (data.avatarUrl !== undefined) extra.avatarUrl = data.avatarUrl || null;
  if (Object.keys(extra).length) return service.prisma.student.update({ where: { id }, data: extra, include: { section: { include: { class: true } } } });
  return result;
};
