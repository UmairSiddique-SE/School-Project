...
export class PeopleService {
    ...
    try {
      return await this.prisma.$transaction(async (tx) => {
        ...
        if (data.fatherName || data.motherName || data.guardianName) {
          ...
          await tx.studentParent.create({ data: { studentId: student.id, parentId: parent.id, isPrimary: true } });
        }
        return student;
      });
    } catch (error: any) {
  ...
  async deleteStaff(id: string, schoolId: string) {
    ...
    if (staff) return this.prisma.$transaction(async (tx) => { await tx.staff.update({ where: { id }, data: { deletedAt: new Date(), isActive: false } }); await tx.user.updateMany({ where: { email: staff.email, schoolId }, data: { isActive: false, deletedAt: new Date() } }); });
    const teacher = await this.prisma.teacher.findFirst({ where: { id, schoolId } });
    if (teacher) return this.deleteTeacher(id, schoolId);
    throw new NotFoundException('Staff member not found');
  }
  ...
  async getSchoolStats(schoolId: string) {
    ...
    const [studentsCount, teachersCount, parentsCount, staffCount, classesCount, pendingHomeworks, pendingLeaves, overdueLibraryBooks, attendances, feePayments, announcements, recentAdmissions, upcomingExams] = await Promise.all([
      ...
      this.prisma.bookIssue.count({ where: { book: { schoolId }, returnDate: null, dueDate: { lt: new Date() } } }),
      this.prisma.attendance.findMany({ where: { schoolId, date: { gte: today, lt: tomorrow }, studentId: { not: null } }, select: { status: true } }),
      this.prisma.feePayment.findMany({ where: { schoolId }, select: { totalPaid: true, amount: true, status: true } }),
      this.prisma.announcement.findMany({ where: { schoolId }, take: 5, orderBy: { publishedAt: 'desc' } }),
      this.prisma.student.findMany({ where: { schoolId, deletedAt: null }, take: 5, orderBy: { admissionDate: 'desc' }, select: { id: true, name: true, admissionDate: true, section: { select: { class: { select: { name: true } } } } } }),
      this.prisma.exam.findMany({ where: { schoolId, deletedAt: null, startDate: { gte: today } }, take: 5, orderBy: { startDate: 'asc' }, select: { id: true, name: true, type: true, startDate: true } }),
    ]);
    const totalTodayAttendance = attendances.length; const presentToday = attendances.filter(a => a.status === 'PRESENT').length; const absentToday = attendances.filter(a => a.status === 'ABSENT').length; const todayAttendancePercentage = totalTodayAttendance > 0 ? Math.round((presentToday / totalTodayAttendance) * 100) : 0;
    const totalRevenue = feePayments.reduce((acc, curr) => acc + (curr.totalPaid || 0), 0); const pendingFees = feePayments.filter((p) => p.status === 'PENDING' || p.status === 'OVERDUE').reduce((acc, curr) => acc + (curr.amount - curr.totalPaid), 0); const pendingFeePaymentsCount = feePayments.filter((p) => p.status === 'PENDING' || p.status === 'OVERDUE').length;
    return { studentsCount, teachersCount, parentsCount, staffCount, classesCount, todayAttendancePercentage, presentToday, absentToday, totalRevenue, pendingFees, pendingFeePaymentsCount, pendingHomeworks, pendingLeaves, overdueLibraryBooks, announcements, recentAdmissions, upcomingExams };
  }
}
