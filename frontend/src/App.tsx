...
const TenantHome = () => { const { user } = useAuth(); const slug = user?.schoolSlug || "edusphere"; if (user?.role === "PARENT") return <Navigate to={`/${slug}/parent-portal`} replace />; if (user?.role === "STUDENT") return <Navigate to={`/${slug}/dashboard`} replace />; return <Navigate to={`/${slug}/dashboard`} replace />; };
