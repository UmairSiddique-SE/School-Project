...
const TenantRedirect = ({ to }: { to: string }) => {
  ...
  if (!isAuthenticated || !user) return <Navigate to="/school-login" replace />;
  if (user.role === 'SUPER_ADMIN') return <Navigate to="/super-admin" replace />;
  if (!user.schoolSlug) return <Navigate to="/unauthorized" replace />;
  return <Navigate to={`/${user.schoolSlug}/${to}`} replace />;
};
...
export default function App() {
  return <ThemeProvider><AuthProvider><BrowserRouter><Routes>
    ...
    {['dashboard','students','teachers','parents','classes','staff','homework','exams','timetable','notices','transport','reports','subscription','attendance','notifications','finance','settings','buildings'].map(path => <Route key={path} path={`/${path}`} element={<TenantRedirect to={path === 'teachers' ? 'staff' : path} />} />)}
