  ...
  useEffect(() => {
    ...
    apiClient
      ...
      .then((response) => {
        ...
        const items = Array.isArray(data) ? data : Array.isArray(data?.schools) ? data.schools : [];
        setSchools(items);
      })
      .catch(() => {
          ...
          {filteredSchools.map((school) => {
            ...
            return (
              <Link
                ...
                to={school.loginAvailable && school.slug ? loginPath : "#"}
                onClick={(event) => {
                  if (!school.loginAvailable || !school.slug) event.preventDefault();
                }}
                className={`rounded-2xl border bg-white p-5 shadow-sm transition hover:-translate-y-0.5 hover:shadow-md ${
                  !school.loginAvailable ? "cursor-not-allowed opacity-60" : ""
                }`}
              >
                ...
                <div className="mt-5 text-sm font-medium text-blue-600">
                  {school.loginAvailable ? "Open school login →" : "Login unavailable"}
                </div>
              </Link>
            );
          })}
