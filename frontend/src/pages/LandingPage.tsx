...
const faqs = [
  ["Can I try EduSphere before paying?", "Yes. The Free Trial plan is designed for schools to experience the core platform before choosing a paid plan."],
  ["Who controls pricing and limits?", "The Super Admin can edit plan price, period, student limit, staff limit, storage, support tier and every feature line from Plans & Pricing."],
  ["Will pricing changes appear on this page?", "Yes. The pricing section is connected to the public plans API, so the landing page reads the live PlatformPlan records from PostgreSQL."],
  ["How are school registrations approved?", "School registration requests go to the Super Admin for review. Approval creates the school account and subscription according to the selected plan."],
...
const planStyle: Record<string, { gradient: string; border: string; icon: React.ComponentType<any>; badge?: string }> = {
  ...
  PROFESSIONAL: { gradient: "from-indigo-500 via-blue-600 to-violet-600", border: "border-indigo-400/40", icon: Star, badge: "MOST POPULAR" },
  ...
  return <nav className="fixed left-0 right-0 top-0 z-50 px-4 pt-4">
    <div className="mx-auto flex max-w-7xl items-center justify-between rounded-2xl border border-white/10 bg-[#071522]/85 px-4 py-3 shadow-2xl backdrop-blur-xl md:px-5">
      ...
      <div className="hidden items-center gap-2 md:flex">
        ...
        <button onClick={() => navigate("/register-school")} className="rounded-xl bg-gradient-to-r from-cyan-500 to-blue-600 px-4 py-2.5 text-xs font-black text-white shadow-lg shadow-cyan-500/20 hover:brightness-110">Register School</button>
      </div>
    ...
    <AnimatePresence>{open && <motion.div initial={{ opacity: 0, y: -8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }} className="mx-4 mt-2 rounded-2xl border border-white/10 bg-[#071522]/95 p-5 backdrop-blur-xl md:hidden"><div className="flex flex-col gap-3">{["Features", "Modules", "Pricing", "FAQ"].map((x) => <a onClick={() => setOpen(false)} key={x} href={`#${x.toLowerCase()}`} className="rounded-xl px-3 py-2 text-sm font-semibold text-slate-200 hover:bg-white/5">{x}</a>)}<button onClick={() => { setOpen(false); navigate("/admin-login"); }} className="flex items-center justify-center gap-2 rounded-xl border border-violet-400/20 bg-violet-500/10 py-2.5 text-sm font-bold text-violet-200 hover:bg-violet-500/20"><ShieldCheck size={15} /> Super Admin</button><button onClick={onSearch} className="rounded-xl border border-white/10 py-2.5 text-sm font-bold text-white">School Login</button><button onClick={() => navigate("/register-school")} className="rounded-xl bg-gradient-to-r from-cyan-500 to-blue-600 py-2.5 text-sm font-black text-white">Register School</button></div></motion.div>}</AnimatePresence>
  </nav>;
}
...
function Pricing() {
  ...
  return <section id="pricing" className="relative px-5 py-24"><div className="absolute inset-x-0 top-1/3 -z-10 mx-auto h-72 max-w-5xl rounded-full bg-indigo-500/10 blur-[120px]" /><div className="mx-auto max-w-7xl"><div className="mx-auto max-w-2xl text-center"><span className="text-[10px] font-black uppercase tracking-[.2em] text-cyan-300">Live pricing</span><h2 className="mt-3 text-4xl font-black text-white sm:text-5xl">Simple plans. <span className="text-cyan-300">Real value.</span></h2><p className="mt-4 text-sm leading-6 text-slate-400">Prices, limits and feature lines below come directly from the Super Admin plan catalogue.</p></div>{loading ? <div className="mt-12 rounded-3xl border border-white/10 bg-white/[.03] p-12 text-center text-sm text-slate-400">Loading live plans…</div> : <div className="mt-12 grid gap-6 lg:grid-cols-3">{plans.map((plan) => { const style = planStyle[plan.planKey] || planStyle.PROFESSIONAL; const Icon = style.icon; const students = plan.maxStudents >= 999999 ? "Unlimited students" : `${plan.maxStudents.toLocaleString()} students`; const staff = plan.maxTeachers >= 999999 ? "Unlimited staff" : `${plan.maxTeachers.toLocaleString()} staff`; return <motion.article key={plan.id} whileHover={{ y:-8 }} className={`relative overflow-hidden rounded-[30px] border ${style.border} bg-[#091a2a]/90 p-7 shadow-2xl shadow-black/20 backdrop-blur-xl ${plan.planKey === "PROFESSIONAL" ? "ring-1 ring-indigo-400/20" : ""}`}><div className={`absolute inset-x-0 top-0 h-1.5 bg-gradient-to-r ${style.gradient}`} />{style.badge && <span className={`absolute right-5 top-5 rounded-full bg-gradient-to-r ${style.gradient} px-2.5 py-1 text-[9px] font-black tracking-wider text-white`}>{style.badge}</span>}<div className={`flex h-12 w-12 items-center justify-center rounded-2xl bg-gradient-to-br ${style.gradient} text-white shadow-lg`}><Icon size={21} /></div><h3 className="mt-6 text-2xl font-black text-white">{plan.name}</h3><p className="mt-2 min-h-10 text-xs leading-5 text-slate-400">{plan.planKey === "FREE_TRIAL" ? "Start with the essentials and experience EduSphere." : plan.planKey === "PROFESSIONAL" ? "For growing schools that need the complete operational stack." : "For schools that want the full platform with maximum scale."}</p><div className="mt-6 flex items-end gap-1">{plan.price === 0 ? <span className="text-4xl font-black text-white">Free</span> : <><span className="mb-1 text-xs font-bold text-slate-500">PKR</span><span className="text-4xl font-black tracking-tight text-white">{plan.price.toLocaleString()}</span></>}<span className="mb-1 text-xs font-bold text-slate-500">{plan.price === 0 ? plan.period : `/${plan.period.replace(/^per /, "")}`}</span></div><div className="mt-6 grid grid-cols-2 gap-2"><div className="rounded-xl bg-white/[.045] p-3"><GraduationCap size={14} className="text-cyan-300" /><p className="mt-2 text-[11px] font-black text-white">{students}</p></div><div className="rounded-xl bg-white/[.045] p-3"><Users size={14} className="text-indigo-300" /><p className="mt-2 text-[11px] font-black text-white">{staff}</p></div></div><ul className="mt-6 space-y-2.5">{(plan.features || []).map((feature, i) => <li key={`${feature}-${i}`} className="flex gap-2 text-xs leading-5 text-slate-300"><Check size={14} className="mt-0.5 shrink-0 text-cyan-300" />{feature}</li>)}</ul><div className="mt-7 flex items-center gap-2 text-[10px] font-bold text-slate-500"><span>{plan.storageMb >= 512000 ? "500 GB" : `${Math.round(plan.storageMb / 1024)} GB`} storage</span><span>•</span><span>{plan.supportTier} support</span></div><button onClick={() => navigate(plan.planKey === "FREE_TRIAL" ? "/register-school" : "/register-school")} className={`mt-7 w-full rounded-2xl bg-gradient-to-r ${style.gradient} px-4 py-3.5 text-sm font-black text-white shadow-lg transition hover:brightness-110`}>{plan.planKey === "FREE_TRIAL" ? "Start Free Trial" : "Get Started"}<ArrowRight size={15} className="ml-2 inline" /></button></motion.article>; })}</div>}</div></section>;
}
