...
import { User, Mail, Phone, Lock, Camera, Check, Loader2, Shield, Bell, Globe, Key } from 'lucide-react';
...
export default function Profile() {
  ...
  const stats = [
    { label: 'Total Schools', value: '24', icon: Globe, color: 'text-violet-400', bg: 'bg-violet-500/10' },
    { label: 'Actions Today', value: '12', icon: Shield, color: 'text-blue-400', bg: 'bg-blue-500/10' },
    { label: 'Notifications', value: '4', icon: Bell, color: 'text-amber-400', bg: 'bg-amber-500/10' },
    { label: 'Last Login', value: 'Today', icon: Key, color: 'text-emerald-400', bg: 'bg-emerald-500/10' },
  ];
  ...
  return (
    <div className="space-y-8 max-w-3xl">
      <div>
        ...
        <p className="text-muted-foreground text-sm mt-1">Manage your super admin account information and security</p>
      </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div>
              ...
              <select value={profile.timezone} onChange={e => setProfile(p => ({ ...p, timezone: e.target.value }))}
                ...
                <option>Asia/Karachi</option><option>UTC</option><option>America/New_York</option><option>Europe/London</option><option>Asia/Dubai</option>
              </select>
            </div>
          </div>
          <button type="submit" disabled={savingProfile}
            className={`flex items-center gap-2 px-5 py-2.5 rounded-xl font-bold text-sm transition-all disabled:opacity-70 ${
              profileSaved ? 'bg-emerald-500 text-white' : 'bg-primary text-primary-foreground hover:bg-primary/90'
            }`}>
            {savingProfile ? <Loader2 size={14} className="animate-spin" /> : profileSaved ? <Check size={14} /> : <User size={14} />}
            {savingProfile ? 'Saving…' : profileSaved ? 'Saved!' : 'Update Profile'}
          </button>
        </form>
      </motion.div>
      ...
      >
        ...
        <form onSubmit={handlePasswordSave} className="space-y-4 max-w-sm">
          ...
          <button type="submit" disabled={savingPassword} className="flex items-center gap-2 px-5 py-2.5 rounded-xl bg-red-500/10 text-red-400 font-bold text-sm hover:bg-red-500/20 transition-all disabled:opacity-70">
