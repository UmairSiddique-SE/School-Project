...
import { Check, Edit2, X, Loader2, Zap, Award, Sparkles, Users, GraduationCap, HardDrive, Headphones } from 'lucide-react';
...
const planMeta: Record<string, { color: string; gradient: string; icon: React.ComponentType<any>; badge?: string }> = {
  ...
  PREMIUM: { color: 'text-amber-400', gradient: 'from-amber-500 to-orange-600', icon: Sparkles, badge: 'Premium' },
};
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-5">
        {plans.map((plan, i) => {
          ...
          return (
            <motion.div key={plan.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.08 }} className="bg-card border border-border rounded-2xl overflow-hidden hover:shadow-xl hover:shadow-primary/5 transition-all">
              <div className={`bg-gradient-to-br ${meta.gradient} p-6 relative`}>
                {meta.badge && <span className="absolute top-3 right-3 text-[10px] font-black px-2 py-0.5 rounded-full bg-white/20 text-white backdrop-blur-sm">{meta.badge}</span>}
                <Icon size={28} className="text-white/80 mb-3" />
                <p className="text-white font-black text-lg">{plan.name}</p>
                <div className="flex items-baseline gap-1 mt-1">
                  {plan.price === 0 ? <span className="text-white text-3xl font-black">Free</span> : <><span className="text-white/70 text-sm">Rs.</span><span className="text-white text-3xl font-black">{plan.price.toLocaleString('en-PK')}</span><span className="text-white/70 text-xs">{plan.period}</span></>}
                </div>
              ...
              <div className="p-5 space-y-4">
                <div className="grid grid-cols-2 gap-2">
                  {[
                    { label: 'Students', value: plan.maxStudents >= 999999 ? '∞' : plan.maxStudents.toLocaleString('en-PK'), icon: GraduationCap },
                    { label: 'Staff', value: plan.maxTeachers >= 999999 ? '∞' : plan.maxTeachers.toLocaleString('en-PK'), icon: Users },
                ...
                <ul className="space-y-1.5">{(plan.features || []).map((f, idx) => <li key={idx} className="flex items-start gap-2 text-xs text-muted-foreground"><Check size={11} className={`mt-0.5 shrink-0 ${meta.color}`} />{f}</li>)}</ul>
                <button onClick={() => handleEdit(plan)} className="w-full flex items-center justify-center gap-2 py-2 rounded-xl border border-border text-xs font-semibold text-muted-foreground hover:bg-accent hover:text-foreground transition-all"><Edit2 size={12} /> Edit Pricing</button>
              </div>
            </motion.div>
          );
        })}
      ...
      <AnimatePresence>
        {editing && <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-50 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4">
          <motion.div initial={{ scale: 0.93, y: 20 }} animate={{ scale: 1, y: 0 }} exit={{ scale: 0.93, y: 20 }} className="bg-card border border-border rounded-2xl p-6 w-full max-w-sm shadow-2xl">
            ...
            <div className="space-y-4">
              {[
                { label: 'Monthly Price (PKR)', key: 'price', type: 'number' },
                { label: 'Max Students', key: 'maxStudents', type: 'number' },
                { label: 'Max Staff', key: 'maxTeachers', type: 'number' },
