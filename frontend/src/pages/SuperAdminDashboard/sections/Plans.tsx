...
export default function Plans() {
  ...
  if (loading) return <div className="flex h-64 items-center justify-center"><Loader2 size={32} className="animate-spin text-primary" /></div>;

  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-2xl font-black text-foreground">Subscription Plans</h2>
        <p className="text-muted-foreground text-sm mt-1">Manage PKR pricing and feature limits for each plan tier</p>
      </div>
            ...
            <motion.div key={plan.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.08 }} className="bg-card border border-border rounded-2xl overflow-hidden hover:shadow-xl hover:shadow-primary/5 transition-all">
              <div className={`bg-gradient-to-br ${meta.gradient} p-6 relative`}>
                ...
                <div className="flex items-baseline gap-1 mt-1">
                  {plan.price === 0 ? <span className="text-white text-3xl font-black">Free</span> : <><span className="text-white/80 text-sm">PKR</span><span className="text-white text-3xl font-black">{plan.price.toLocaleString()}</span><span className="text-white/70 text-xs">/{plan.period === 'per month' ? 'month' : plan.period}</span></>}
                </div>
              ...
              <div className="p-5 space-y-4">
                ...
                <ul className="space-y-1.5">
                  {(plan.features || []).map((f, idx) => <li key={idx} className="flex items-start gap-2 text-xs text-muted-foreground"><Check size={11} className={`mt-0.5 shrink-0 ${meta.color}`} />{f}</li>)}
                </ul>
                <button onClick={() => handleEdit(plan)} className="w-full flex items-center justify-center gap-2 py-2 rounded-xl border border-border text-xs font-semibold text-muted-foreground hover:bg-accent hover:text-foreground transition-all"><Edit2 size={12} /> Edit Pricing & Limits</button>
              </div>
            </motion.div>
          );
        })}
