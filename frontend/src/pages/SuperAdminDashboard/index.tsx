...
import { LayoutDashboard, School, FileText, CreditCard, BarChart2, Bell, Mail, Settings, Shield, User, Menu, X, ChevronRight, LogOut, Clock, Zap, LifeBuoy, Megaphone, Sun, Moon, Plus } from "lucide-react";
...
import ManualPaymentDialog from "./sections/ManualPaymentDialog";
...
export default function SuperAdminDashboard() {
    ...
    <>
      <div className="relative shrink-0 overflow-hidden px-5 py-5">
        ...
        <div className="relative flex items-center justify-between">
          <div className="flex items-center gap-3"><div className="relative flex h-10 w-10 items-center justify-center rounded-xl bg-gradient-to-br from-violet-500 to-indigo-600 shadow-lg shadow-violet-500/30"><Shield size={17} className="text-white" /><span className="absolute -right-0.5 -top-0.5 h-2.5 w-2.5 rounded-full border-2 border-card bg-emerald-400" /></div><div><p className="text-sm font-black leading-none tracking-tight text-foreground">EduSphere</p><p className="mt-1 text-[10px] font-bold uppercase tracking-[0.18em] text-primary">Multi-Tenant SaaS</p></div></div>
  ...
  return <div className="edusphere-admin relative flex h-full min-h-0 w-full overflow-hidden rounded-[28px] border border-border bg-background text-foreground shadow-[0_20px_60px_rgba(15,23,42,0.18)] transition-colors duration-300">
    ...
    <ManualPaymentDialog open={manualPaymentOpen} onClose={() => setManualPaymentOpen(false)} onSaved={() => window.location.reload()} />
  </div>;
}
