    ...
    <div className="space-y-6">
      <div className="flex justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-muted-foreground">Live timetable records for the current school.</p>
        </div>
        <button onClick={() => void load()} className="px-4 py-2 rounded-lg border flex items-center gap-2">
      ...
      {canManage && (
        <form onSubmit={create} className="rounded-xl border p-5 grid md:grid-cols-3 lg:grid-cols-6 gap-3">
          <select className="rounded-lg border p-3 bg-background" value={form.dayOfWeek} onChange={e => setForm({ ...form, dayOfWeek: e.target.value })}>
          ...
          <select required className="rounded-lg border p-3 bg-background" value={form.sectionId} onChange={e => setForm({ ...form, sectionId: e.target.value })}>
            <option value="">Section</option>
            {sections.map(s => <option key={s.id} value={s.id}>{s.class?.name} / {s.name}</option>)}
          </select>
          <select required className="rounded-lg border p-3 bg-background" value={form.subjectId} onChange={e => setForm({ ...form, subjectId: e.target.value })}>
            <option value="">Subject</option>
            {subjects.map(s => <option key={s.id} value={s.id}>{s.name}</option>)}
          </select>
          <select className="rounded-lg border p-3 bg-background" value={form.teacherId} onChange={e => setForm({ ...form, teacherId: e.target.value })}>
          ...
          <button disabled={saving} className="md:col-span-2 lg:col-span-5 rounded-lg bg-primary text-primary-foreground py-3">
            <Plus size={16} className="inline mr-1" />{saving ? 'Saving...' : 'Add Slot'}
          </button>
        </form>
      )}
      ...
      {loading ? (
        ...
        <div className="rounded-xl border border-dashed p-10 text-center text-muted-foreground">No timetable slots have been created.</div>
      ) : (
        <div className="rounded-xl border overflow-x-auto">
          <table className="w-full text-sm">
            <thead><tr className="border-b text-left"><th className="p-3">Day</th><th className="p-3">Time</th><th className="p-3">Class / Section</th><th className="p-3">Subject</th><th className="p-3">Teacher</th><th className="p-3">Room</th>{canManage && <th className="p-3">Action</th>}</tr></thead>
            <tbody>{slots.map(s => <tr key={s.id} className="border-b last:border-0"><td className="p-3">{day(s.dayOfWeek)}</td><td className="p-3">{s.startTime}–{s.endTime}</td><td className="p-3">{s.section?.class?.name || '—'} / {s.section?.name || '—'}</td><td className="p-3">{s.subject?.name || '—'}</td><td className="p-3">{s.teacher?.name || '—'}</td><td className="p-3">{s.room || '—'}</td>{canManage && <td className="p-3"><button onClick={() => void remove(s.id)} className="text-destructive"><Trash2 size={16} /></button></td>}</tr>)}</tbody>
          </table>
        </div>
      )}
    </div>
