...
import { ArrowRightLeft, ArrowUpRight, Camera, Download, FileUp, Loader2, Pencil, Plus, Printer, RefreshCw, Trash2, UserRound, Users, X } from 'lucide-react';
...
export default function StudentsFull() {
  ...
  const sections = useMemo<Section[]>(() => classes.flatMap((item) => (item.sections || []).map((section: any) => ({
    ...section,
    className: item.name,
    classId: item.id,
  }))), [classes]);
  ...
  const filtered = useMemo(() => {
    ...
    return students.filter((student) => [
      student.name, student.admissionNo, student.rollNo, student.email, student.phone, student.religion,
      student.admissionType, student.previousSchool, student.section?.name, student.section?.class?.name,
  ...
  const handlePhoto = async (file?: File) => {
    ...
    try {
      ...
      toast.success('Student photo prepared');
    } catch (error: any) {
  ...
  const save = async (event: React.FormEvent) => {
    ...
    if (form.admissionType === 'TRANSFER' && (!form.previousSchool.trim() || !form.previousClass.trim())) {
      toast.error('Previous School and Previous Class are required for transfer students');
      return;
    }
    ...
    try {
      const payload: any = {
        ...
        studentMobile: form.phone.trim() || undefined,
        email: form.email.trim().toLowerCase() || undefined,
        address: form.currentAddress.trim() || form.address.trim() || undefined,
  ...
  const exportCsv = () => {
    const rows = [
      ...
      ...students.map((student) => [student.admissionNo, student.name, student.section?.class?.name, student.section?.name, student.rollNo, student.religion, student.admissionType, student.previousSchool, student.previousClass, student.fatherName, student.fatherMobile1]),
    ];
  ...
  return <div className="space-y-6">
    <div className="flex flex-col gap-4 xl:flex-row xl:items-end xl:justify-between">
      <div>
        <p className="text-[10px] font-black uppercase tracking-[0.24em] text-violet-500">People Management</p>
        <h1 className="mt-1 text-3xl font-black tracking-tight text-foreground">Students</h1>
        <p className="mt-1 text-sm text-muted-foreground">Live admissions, student records and academic movement.</p>
      </div>
      <div className="flex flex-wrap gap-2">
        <button className={button} onClick={() => void load()}><RefreshCw size={15}/></button>
        <button className={button} onClick={exportCsv}><Download size={15}/> Export</button>
        <label className={`${button} cursor-pointer`}><FileUp size={15}/>{importing ? 'Importing…' : 'Import CSV'}<input type="file" accept=".csv,text/csv" className="hidden" disabled={importing} onChange={(event) => { const file = event.target.files?.[0]; if (file) void importCsv(file); event.currentTarget.value = ''; }}/></label>
        <button className={primary} onClick={openCreate}><Plus size={16}/> Add Student</button>
      </div>
    </div>

    <div className="grid grid-cols-2 gap-3 xl:grid-cols-4">
      <Stat label="Total Students" value={students.length}/><Stat label="Active" value={activeCount}/><Stat label="Transfers" value={transferCount}/><Stat label="Used Sections" value={new Set(students.map((student) => student.sectionId).filter(Boolean)).size}/>
    </div>

    <div className="rounded-3xl border border-border bg-card p-4 shadow-sm">
      <div className="flex flex-col gap-3 lg:flex-row lg:items-center">
        <input className={input} value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Search student, admission no, class, section or parent..."/>
        {selectedIds.length > 0 && <div className="flex flex-wrap gap-2"><button className={button} onClick={() => setMoveMode('promote')}><ArrowUpRight size={15}/> Promote</button><button className={button} onClick={() => setMoveMode('transfer')}><ArrowRightLeft size={15}/> Transfer</button><button className={button} onClick={() => selectedIds.map((id) => students.find((student) => student.id === id)).filter(Boolean).forEach((student) => printId(student))}><Printer size={15}/> Print ID</button></div>}
      </div>
    </div>
    ...
    {formOpen && <Modal title={editing ? 'Edit Student Profile' : 'Student Admission'} onClose={() => setFormOpen(false)}>
      <form onSubmit={save} className="space-y-7">
        <SectionTitle title="Student Information" subtitle="Basic identity, photo and identification details."/>
        <div className="grid gap-4 rounded-2xl border border-border bg-muted/20 p-4 md:grid-cols-[1fr_auto] md:items-center">
          <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">
            ...
            <Field label="B-Form / CNIC"><input className={input} placeholder="35202-*******-*" value={form.bFormNumber} onChange={(event) => update('bFormNumber', event.target.value)}/></Field>
            ...
            <Field label="Student Mobile"><input className={input} value={form.phone} onChange={(event) => update('phone', event.target.value)}/></Field>
          </div>
          <label className="mx-auto flex h-28 w-28 cursor-pointer items-center justify-center overflow-hidden rounded-2xl border-2 border-dashed border-border bg-background">
            <input type="file" accept="image/*" className="hidden" disabled={compressing} onChange={(event) => { const file = event.target.files?.[0]; if (file) void handlePhoto(file); event.currentTarget.value = ''; }}/>
            {form.avatarUrl ? <img src={form.avatarUrl} alt="Student" className="h-full w-full object-cover"/> : <div className="flex flex-col items-center gap-2 text-muted-foreground"><Camera size={24}/><span className="text-[10px] font-bold">Upload Photo</span></div>}
          </label>
        </div>

        <SectionTitle title="Academic & Class" subtitle="Assign the student to a live class and section."/>
        <div className="grid gap-4 rounded-2xl border border-border p-4 md:grid-cols-3">
          ...
          <div className="md:col-span-3 rounded-xl border border-violet-500/20 bg-violet-500/5 p-4"><p className="text-[10px] font-black uppercase tracking-wider text-violet-600">Class Teacher</p><p className="mt-1 text-sm font-bold">{selectedSection?.teacher?.name || 'Not assigned for this section'}</p><p className="mt-1 text-xs text-muted-foreground">Teacher comes directly from the selected live section.</p></div>
        </div>
...
function SectionTitle({ title, subtitle }: { title: string; subtitle: string }) {
  return <div className="border-b border-border pb-2"><h3 className="text-base font-black text-foreground">{title}</h3><p className="mt-1 text-xs text-muted-foreground">{subtitle}</p></div>;
}

function Field({ label, required, children }: { label: string; required?: boolean; children: React.ReactNode }) {
  return <label className="block"><span className="mb-1.5 block text-[10px] font-black uppercase tracking-[0.14em] text-muted-foreground">{label}{required ? ' *' : ''}</span>{children}</label>;
}

function Info({ label, value }: { label: string; value: unknown }) {
  return <div className="rounded-xl border border-border bg-muted/20 p-3"><p className="text-[9px] font-black uppercase tracking-[0.14em] text-muted-foreground">{label}</p><p className="mt-1 break-words text-sm font-semibold">{String(value ?? '—') || '—'}</p></div>;
}

function Stat({ label, value }: { label: string; value: number }) {
  return <div className="rounded-2xl border border-border bg-card p-4 shadow-sm"><p className="text-2xl font-black">{value}</p><p className="mt-1 text-[10px] font-bold uppercase tracking-[0.14em] text-muted-foreground">{label}</p></div>;
}

function Modal({ title, onClose, children }: { title: string; onClose: () => void; children: React.ReactNode }) {
  return <div className="fixed inset-0 z-[80] flex items-center justify-center bg-black/60 p-4 backdrop-blur-sm"><div className="max-h-[94vh] w-full max-w-6xl overflow-y-auto rounded-3xl border border-border bg-card shadow-2xl"><div className="sticky top-0 z-20 flex items-center justify-between border-b border-border bg-card px-6 py-4"><h2 className="text-lg font-black">{title}</h2><button onClick={onClose} className="rounded-lg p-2 text-muted-foreground hover:bg-muted"><X size={18}/></button></div><div className="p-6">{children}</div></div></div>;
}
